...
import { RNCamera, Face, Barcode, TrackedTextFeature, Point, RecordOptions, FlashMode, WhiteBalance, AutoFocus, CameraType, BarCodeType, Size, BarcodeType } from 'react-native-camera';
...
pauseCamera = () => this.setState({ displayCamera: false })
  ...
  toggleFacing = () => {
    this.setState(previousState => ({
      type: previousState.type === RNCamera.Constants.Type.back ? RNCamera.Constants.Type.front : RNCamera.Constants.Type.back,
    }));
  }

  setFlash = (flash: keyof FlashMode) => {
    this.setState({ flash });
  }
  toggleFlash = () => {
  ...
  setAutofocus = (autoFocus: keyof AutoFocus) => {
      this.setState({ autoFocus });
  }
  toggleFocus = () => {
    this.setState(previousState => ({
      autoFocus: previousState.autoFocus === RNCamera.Constants.AutoFocus.on ? RNCamera.Constants.AutoFocus.off : RNCamera.Constants.AutoFocus.on,
    }));
  }

  touchToFocus = (event: GestureResponderEvent) => () => {
  ...
  toggle = (value: 'canDetectFaces' | 'canDetectText' | 'canDetectBarcode') => {
  ...
  facesDetected = (response: { faces: Face[] }) => this.setState({ faces: response.faces });

  renderFace = ({ bounds, faceID, rollAngle, yawAngle }: Face) => (
    <View
      key={faceID}
      // transform={[
      //   { perspective: 600 },
  ...
  renderLandmarksOfFace = (face: Face) => {
    const renderLandmark = (position?: Point<number>) =>
      position && (
        <View
          style={[
            styles.landmark,
  ...
  renderTextBlock = ({ bounds, value } : TrackedTextFeature) => (
  ...
  textRecognized = (response: { textBlocks: TrackedTextFeature[] }) => {
    const { textBlocks } = response;
    this.setState({ textBlocks });
  };

  legacyBarcodeToBarcode = (legacy: LegacyBarcode) => {
    let width: number = 0;
    let height: number = 0;
    let x: number = 0;
    let y: number = 0;
  ...
  barcodeRecognized = (event: LegacyBarcode) =>  {
    const barcode = this.legacyBarcodeToBarcode(event)
    let previousBarcode: Barcode|undefined = this.state.barcodes.length > 0 ? this.state.barcodes[0] : undefined;
    this.setState({ barcodes: [barcode] });
    if (!previousBarcode || previousBarcode!.data !== barcode.data) {
      this.beepSound.play(() => {
  ...
  renderBarcode = ({ bounds, data, type }: Barcode) => (
  ...
  renderManualSearchView() {
    return (
        <DialogInput isDialogVisible={this.state.showManualSearchView}
            ...
            submitInput={(barcodeData: string) => { 
