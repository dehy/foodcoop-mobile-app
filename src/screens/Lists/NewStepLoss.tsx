...
export default class ListsNewStepLoss extends React.Component<Props, State> {
    theme = {
        Button: {
    ...
    createListAndDismiss = (): void => {
        ...
        getConnection().getRepository(List).save(list).then(() => {
    ...
    render() {
        return (
            <ThemeProvider theme={this.theme}>
                <SafeAreaView>
                    <InventoryIcon height={200} width='100%' style={{padding: 10}} />
                    <ListItem>
                        <ListItem.Content>
                            <ListItem.Title>Zone</ListItem.Title>
                        </ListItem.Content>
                        <ListItem.Content right>
                            <ListItem.Input placeholder='Zone' autoFocus={true} onChangeText={(text) => {
                                this.zoneValue = parseInt(text);
                            }} blurOnSubmit={true} onSubmitEditing={(e) => {
                    ...
                    <View>
                        <Button title="Commencer l'inventaire" style={{margin:20}} onPress={() => {
                            this.createListAndDismiss();
                        }} ref={(ref) => { this.submitButton = ref ?? undefined }} />
                    </View>
                </SafeAreaView>
            </ThemeProvider>
        )
