...
export default class PurchaseOrderFactory {
    public static PurchaseOrderFromResponse(response: OdooApiPurchaseOrder): PurchaseOrder {
        ...
        purchaseOrder.orderDate = moment(response.date_order, Odoo.DATETIME_FORMAT).toDate();
        purchaseOrder.plannedDeliveryDate = moment(response.date_planned, Odoo.DATETIME_FORMAT).toDate();
