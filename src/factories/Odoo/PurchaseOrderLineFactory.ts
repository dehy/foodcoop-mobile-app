...
export default class PurchaseOrderLineFactory {
  public static PurchaseOrderLineFromResponse(
    response: OdooApiPurchaseOrderLine
  ): PurchaseOrderLine {
    const purchaseOrderLine = new PurchaseOrderLine();
    purchaseOrderLine.id = response.id;
    purchaseOrderLine.name = response.name;
    purchaseOrderLine.productId = response.product_id![0];
    ...
    purchaseOrderLine.productUom = response.product_uom![0];
